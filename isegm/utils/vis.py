...
def draw_points_binary(image, points, radius=3):
    image = image.copy()
    for p in points:
        if p[0] < 0:
            continue
        if len(p) == 3:
            ...
            pradius = radius
        image = cv2.circle(image, (int(p[1]), int(p[0])), pradius, 255, -1)
...
def draw_with_clicks(img, mask=None, alpha=0.6, clicks_list=None, pos_color=(0, 255, 0),
                               neg_color=(255, 0, 0), radius=4):
    # result = img.copy()
    h, w = img.shape[0], img.shape[1]
    pos_click = np.zeros([h, w], dtype='uint8')
    neg_click = np.zeros([h, w], dtype='uint8')

    if clicks_list is not None and len(clicks_list) > 0:
        pos_points = [click.coords for click in clicks_list if click.is_positive]
        neg_points = [click.coords for click in clicks_list if not click.is_positive]
        
        pos_click = draw_points_binary(pos_click, pos_points, radius=radius)
        neg_click = draw_points_binary(neg_click, neg_points, radius=radius)

    # for click maps
    return pos_click, neg_click
