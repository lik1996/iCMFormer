...
class BlockSA(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., mlp_drop=0., qkv_bias=False, attn_drop=0., 
        ...
        self.attn = MHAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, 
            proj_drop=proj_drop)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=mlp_drop)
...
class BlockCA_plus(nn.Module):
    ...
    def forward(self, x, y, z):
        x = x + self.attn1(self.norm1(x), self.norm1(x))
        x = x + self.attn2(self.norm2(y + z), self.norm2(x))
        x = x + self.mlp(self.norm3(x))
        return x
...
class VisionTransformer_cross_encdecv4(nn.Module):
    """ Vision Transformer with support for global average pooling, new architecture for two different inputs
    """    
    def __init__(self, img_size=(224,224), patch_size=(16, 16), in_chans=3, num_classes=1000, embed_dim=768, 
        ...
        #     BlockSA(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, 
        #       attn_drop=attn_drop_rate, proj_drop=proj_drop_rate, norm_layer=norm_layer, 
        #       act_layer=act_layer)
        #     for _ in range(depth // 2)])

        self.blocks2 = nn.Sequential(*[
            BlockCA(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, 
              attn_drop=attn_drop_rate, proj_drop=proj_drop_rate, norm_layer=norm_layer, 
              act_layer=act_layer)
    ...
    def forward_backbone_cross_encdec(self, x, additional_features=None, shuffle=False):
        x = self.patch_embed(x)
        
        x = self.pos_drop(x + self.pos_embed[:, 1:])
        additional_features = self.pos_drop(additional_features + self.pos_embed[:, 1:])
        ...
        if shuffle:
            ...
            for i in range(1, num_blocks // 2 + 1):
                if i % num_blocks_per_group:
                    ...
                    is_patchified = False
                x = self.blocks[i-1](x)
                additional_features = self.blocks[i-1](additional_features)
            
            for j in range(1, num_blocks // 6 + 2):
                if j % num_blocks_per_group:
                    if not is_patchified:
                        x = self.patchify(x)
                        additional_features = self.patchify(additional_features)
                        is_patchified = True
                    ...
                    additional_features = self.unpatchify(additional_features)
                    is_patchified = False
                additional_features = self.blocks2[j-1](additional_features, x)
            
            x = x + additional_features
            for tt in range(num_blocks // 2 + 1, num_blocks + 1):
                if tt % num_blocks_per_group:
                    if not is_patchified:
                        x = self.patchify(x)
                        is_patchified = True
                    else:
